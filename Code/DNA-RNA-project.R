...
#Observation: By overlaying the two curves representing M values for WT and MUT, we observe a slight shift in the mutant curve toward the center. This suggests that there are 
#more unmethylated sites in the MUT group compared to the WT. Looking at the density plot of the beta values, the WT samples show more pronounced peaks near 0 and 1. This indicates that in wild-type samples, there are more CpG sites that are
#either fully methylated or completely unmethylated across all cells. In contrast, the curve for the MUT group appears slightly more centered, which suggests a higher proportion of CpG sites with intermediate
#methylation levels—meaning that in mutants, some cells are methylated at a given site while others are not.
